...
class _ProfileState extends State<Profile> {
  ...
  Future<void> _save(ProfileProvider p) async {
    ...
    if (pwError != null) {
      showMessage(pwError, context, status: MessageStatus.error, title: 'Validation');
      return;
    }
    ...
    if (success) {
      ...
      showMessage(p.errorMessage ?? 'Update failed.', context,
        ...
        builder: (context, p, userProvider, _) {
          ...
          return Scaffold(
            appBar: AppBar(
              ...
              title: const Text(
                'Update Profile',
                style: TextStyle(fontSize: 15, fontWeight: FontWeight.w500, fontFamily: 'Lexend'),
              ),
            ),
            body: SafeArea(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Center(
                      child: Stack(
                        ...
                        children: [
                          ...
                          if (p.imageFile != null || userProvider.hasAvatar)
                            Positioned(
                              right: 0,
                              top: 6,
                    ...
                    if (user != null) ...[
                      ...
                      _readonlyField(user.fullname?.isNotEmpty == true ? user.fullname! : user.name),
                      const SizedBox(height: 18),
                      ...
                      Row(
                        children: [
                          ...
                          if (user.rank?.isNotEmpty == true && user.clinicalRoleLabel.isNotEmpty)
                            const SizedBox(width: 12),
                    ...
                    _fieldLabel('Address'),
                    const SizedBox(height: 8),
                    _customField(controller: p.addressCtrl, hint: 'Street address'),
                          ...
                          child: Row(
                            children: [
                              ...
                              Text(
                                ...
                                style: TextStyle(fontSize: 12, color: Colors.red.shade400, fontFamily: 'Lexend'),
                              ),
                            ],
                          ),
                        ),
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              _fieldLabel('City'),
                              const SizedBox(height: 8),
                              _customField(controller: p.cityCtrl, hint: 'City'),
                            ],
                          ),
                        ),
                      ],
                    ...
                    const SizedBox(height: 8),
                    _customField(controller: p.passCtrl, hint: '••••••••', obscure: true),

                    const SizedBox(height: 18),

                    _fieldLabel('Confirm New Password'),
                    const SizedBox(height: 8),
                    _customField(controller: p.confirmCtrl, hint: '••••••••', obscure: true),
                        ...
                        children: [
                          SizedBox(
                            ...
                            child: ElevatedButton(
                              onPressed: p.isUpdating ? null : () => _save(p),
  ...
  Widget _fieldLabel(String text) => Text(
    text,
    style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w500, fontFamily: 'Lexend'),
  );
  ...
  Widget _readonlyField(String value) => Container(
    ...
    child: Text(
      value,
      style: TextStyle(fontSize: 13, fontFamily: 'Lexend', color: Theme.of(context).hintColor),
    ),
  );
      ...
      Container(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            hintText: hint,
            hintStyle: TextStyle(color: Colors.grey.shade400, fontSize: 13, fontFamily: 'Lexend'),
            prefixIcon: prefix,
  ...
  Widget _imageButton({required IconData icon, required String text, required VoidCallback onTap}) =>
      GestureDetector(
        onTap: onTap,
        child: Container(
          ...
          child: Row(
            children: [
              Icon(icon, size: 18),
              const SizedBox(width: 8),
              Text(text, style: const TextStyle(fontSize: 12, fontFamily: 'Lexend', fontWeight: FontWeight.w500)),
            ],
          ),
        ),
      );
